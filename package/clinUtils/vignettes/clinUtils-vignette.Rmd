...
The `getClinDT` function is an utility function containing sensitive default settings
and extra common
functionalities to include in this table, e.g. possibility to expand
row variable(s), to include bar visualization, ...
...
```{r getClinDT}
	...
	# TODO: this will be moved to the medicalMonitoring package
...
library(plotly)
listPlotsInteractiveLB <- sapply(listPlotsLB, ggplotly, simplify = FALSE)
...
Please note that the `tagList` function of the `htmltools` package 
can also be used to include multiple interactive plots with an associated 
title in a Rmarkdown report.   
However, in this case the titles are not automatically numbered.
